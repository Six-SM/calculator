...
import org.ssm.server.db.DatabaseStorage
...
fun Application.configureRouting(url: String, username: String, password: String) {
    routing {
        ...
        get("/api/history") {
            val response = json.encodeToString(DatabaseStorage(url, username, password).getHistory())
        ...
        post("/api/calculate") {
            ...
            DatabaseStorage(url, username, password).save(calculationRequest, calculationResponse)
